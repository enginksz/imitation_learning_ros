...
class ReplayMemory(Dataset):
  ...
  def wrap_for_absorbing_states(self):
    ...
    # Replace terminal state with absorbing state and remove terminal
    self.next_states[(self.idx - 1) % self.size], self.terminals[(self.idx - 1) % self.size] = absorbing_state, False
    # Add absorbing state pair as next transition
    self.append(self.step[(self.idx - 1) % self.size], absorbing_state, torch.zeros(self.actions.size(1)), 0, absorbing_state, False, False)
